...
import { useUploadSeminarDocument } from '@/hooks/thesis-seminar';
import type {
  SeminarRequirement,
  SeminarRequirementConfiguration,
  SeminarUploadConfig,
...
interface StudentThesisSeminarDocumentCardProps {
  requirements: SeminarRequirement[];
  canUpload: boolean;
  configuration: SeminarRequirementConfiguration;
...
export function StudentThesisSeminarDocumentCard({
  ...
  configuration,
  uploadConfig,
}: StudentThesisSeminarDocumentCardProps) {
  ...
  return (
    <div className="bg-card border border-gray-200 rounded-[10px] p-[16px_18px]">
      <div className="flex items-center justify-between gap-3 mb-[14px]">
        <div className="text-base font-semibold text-foreground">Upload Dokumen Seminar</div>
        <span className="text-xs text-muted-foreground">PDF, maks. {uploadConfig.maxFileSizeMb} MB</span>
      </div>

      {!configuration.isConfigured ? (
        <Notice>{configuration.message || 'Syarat dokumen seminar belum dikonfigurasi.'}</Notice>
      ...
      {configuration.isConfigured && requirements.length === 0 ? (
        <p className="text-sm text-muted-foreground">Belum ada syarat dokumen untuk percobaan seminar ini.</p>
      ) : (
...
interface DocumentRowProps {
  requirement: SeminarRequirement;
  canUpload: boolean;
  uploadConfig: SeminarUploadConfig;
  isUploading: boolean;
...
function DocumentRow({ requirement, canUpload, uploadConfig, isUploading, onUpload }: DocumentRowProps) {
  ...
  const handleView = async () => {
    ...
    try {
      await openProtectedFile(document.filePath, document.fileName);
    } catch (error) {
