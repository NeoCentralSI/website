...
export function YudisiumDetailParticipantsPanel({ yudisiumId }: Props) {
  const navigate = useNavigate();
  const { isAdmin, hasAnyRole } = useRole();
  const { data, isLoading, isFetching, refetch } = useYudisiumParticipants(yudisiumId);
        ...
        <div className="flex gap-1">
          ...
          {canValidateCpl && row.status === 'under_review' && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/yudisium/${yudisiumId}/peserta/${row.id}`)}
              title="Validasi CPL"
            >
              <ShieldCheck className="h-4 w-4 mr-1" />
              Validasi CPL
