...
import type { CpmkWithRubrics } from '@/services/master-data/seminar-rubric.service';
...
export function SeminarRubricPreviewDialog({
    open,
    onOpenChange,
    cpmks,
    minimumPassingScore,
    ...
    const totalMaxScore = configuredCpmks.reduce(
        (total, cpmk) => total + cpmk.assessmentCriterias.reduce(
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
                <DialogHeader className="pr-8">
                    <DialogTitle>Preview Form Penilaian Seminar</DialogTitle>
                    <DialogDescription>
                        Tampilan berikut menggambarkan form yang akan digunakan dosen penguji.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_260px]">
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between gap-2 border-b pb-3">
                            ...
                            <span className="text-xs text-muted-foreground">Penguji: Nama Dosen Penguji</span>
                        </CardHeader>
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            {criterion.assessmentRubrics.length > 0 && (
                                                                <Collapsible
                                                                    ...
                                                                    onOpenChange={(rubricOpen) => setOpenRubrics((previous) => ({
                                                                        ...previous,
                    ...
                    <div className="space-y-4">
                        <Card className="p-6 text-center">
                            <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Total Skor</span>
                            <div className="mt-2 text-5xl font-black">0</div>
                            <span className="mt-1 block text-xs font-semibold text-muted-foreground">/ {totalMaxScore}</span>
                            <Badge variant="secondary" className="mt-4 font-semibold">Belum diisi</Badge>
                        </Card>
                        <Card>
                            <CardContent className="p-4 text-xs">
                                <p className="font-semibold">Batas kelulusan akhir</p>
                                <p className="mt-1 text-muted-foreground">
                                    Minimum rata-rata nilai penguji:{' '}
