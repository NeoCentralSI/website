...
import type { StudentDefenceOverview, ThesisDefenceStatus } from '@/types/defence.types';
import { cn } from '@/lib/utils';

const FINALIZED_STATUSES: ThesisDefenceStatus[] = ['passed', 'passed_with_revision', 'failed'];

const SCHEDULED_STATUSES: ThesisDefenceStatus[] = [
  'scheduled',
  'ongoing',
  'passed',
...
interface DefenceInfoCardProps {
  defence: NonNullable<StudentDefenceOverview['defence']>;
  onClick?: () => void;
}
...
export function StudentThesisDefenceIdentityCard({ defence, onClick }: DefenceInfoCardProps) {
  ...
  if (showSchedule && (defence.room || isOnline)) {
    blocks.push(
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-[14px]">
        <div className="text-base font-semibold text-foreground">Informasi Sidang</div>
        <div className="flex items-center gap-2">
          <ThesisEventStatusBadge
            status={defence.status}
            scheduledDate={defence.date}
