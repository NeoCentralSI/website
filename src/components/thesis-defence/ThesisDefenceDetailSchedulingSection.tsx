...
export function AdminThesisDefenceSchedulingSection({ defenceId, isEditable }: Props) {
  ...
  const confirmDownloadInvitation = () => {
    doDownloadInvitation({ defenceId, nomorSurat: inputNomorSurat });
  ...
  const handleCopyMessage = () => {
    ...
    const studentNim = defenceDetail?.student?.nim || '-';
    const title = defenceDetail?.thesis?.title || '-';
    ...
    const message = `Berikut ini adalah informasi dan jadwal Sidang Tugas Akhir yang akan ditetapkan:
...
Mohon konfirmasinya untuk mensegerakan kelangsungan Sidang Tugas Akhir mahasiswa kita tersebut.`;
            ...
            <CardHeader className="pb-3 shrink-0">
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-3">
                  {defenceDetail?.status === 'examiner_assigned' && current?.date && (
                  ...
                  {['scheduled', 'ongoing', 'passed', 'passed_with_revision', 'failed', 'cancelled'].includes(defenceDetail?.status as string) && (
                  ...
                  {schedulingData?.rooms?.length > 0 && (
                    <Select
                      ...
                      disabled={['scheduled', 'ongoing', 'passed', 'passed_with_revision', 'failed', 'cancelled'].includes(defenceDetail?.status as string)}
                    >
      ...
      <Dialog open={showFinalizeConfirm} onOpenChange={setShowFinalizeConfirm}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-2 text-xs sm:text-sm text-muted-foreground">
            <p>Anda akan menjadwalkan Sidang TA berikut:</p>
            ...
            <div className="space-y-3 bg-muted/40 rounded-lg border p-4 font-medium text-foreground">
              <div>
                ...
                <span className="block mt-0.5 text-xs sm:text-sm font-semibold">
                  {toTitleCaseName(defenceDetail?.student?.name)} ({defenceDetail?.student?.nim})
                </span>
              </div>

              <div>
                <span className="text-xs text-muted-foreground block font-normal">Judul TA</span>
                <span className="block mt-0.5 text-xs sm:text-sm leading-snug font-medium">{defenceDetail?.thesis?.title || '-'}</span>
              </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t">
            ...
            <Button
              disabled={isFinalizing}
              onClick={() => {
                doFinalizeSchedule(defenceId, {
                  onSuccess: () => {
                    toast.success('Jadwal sidang berhasil ditetapkan.');
                    setShowFinalizeConfirm(false);
                  },
                  onError: () => toast.error('Gagal menetapkan jadwal sidang.'),
                });
              }}
