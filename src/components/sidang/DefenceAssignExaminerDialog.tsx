...
import { useDefenceEligibleExaminers, useAssignDefenceExaminers } from '@/hooks/defence';
import { toTitleCaseName } from '@/lib/text';
import { toast } from 'sonner';
import { SearchIcon, Lock } from 'lucide-react';
import type { AssignmentDefenceItem } from '@/types/defence.types';

interface DefenceAssignExaminerDialogProps {
  open: boolean;
...
export function DefenceAssignExaminerDialog({
  open,
  onOpenChange,
  defence,
  onSuccess,
}: DefenceAssignExaminerDialogProps) {
  ...
  const lockedExaminers = useMemo(() => {
    if (!defence) return [];
    return defence.examiners.filter((e) => e.availabilityStatus === 'available');
  }, [defence]);
