  ...
  useEffect(() => {
    ...
    async function setup() {
      try {
        ...
        if (!token) {
          console.warn("[FCM] FCM token not available — Firebase may not be configured");
          return;
        }
        ...
        if (navigator?.serviceWorker) {
          swListener = (event: MessageEvent) => {
            try {
              ...
              // Ignore malformed service-worker messages; notification polling still refreshes counts.
            }
          };
    ...
    const unlock = () => {
      try {
        ...
        // Ignore audio unlock failures in browsers that block AudioContext.
      }
    ...
    return () => {
      ...
      try {
        ...
        // Ignore cleanup failures from already-closed AudioContext instances.
      }
